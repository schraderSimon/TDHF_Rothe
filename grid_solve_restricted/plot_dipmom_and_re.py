...
name_Rothe="Rothe_wavefunctions%.4f_%d_%d_%d_%s.npz"%(fieldstrengh,int(sys.argv[2]),int(sys.argv[3]),int(sys.argv[4]),molecule)
...
len_Rothe=min(len(errors_Rothe),len(errors_gauss))
print("Cumulative Rothe error (linear):",np.sum(errors_gauss[:len_Rothe]))
print("Cumulative Rothe error (full):",np.sum(errors_Rothe[:len_Rothe]))
