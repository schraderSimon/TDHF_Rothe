...
class Rothe_evaluator:
    ...
    def rothe_plus_gradient(self,nonlin_params_unfrozen,printing=False,calculate_overlap=False,include_overlap_correction=True,return_overlap_indices=True):
        ...
        if calculate_overlap or include_overlap_correction or return_overlap_indices:
            ...
            if printing:
                ...
                max_indices=np.unravel_index(np.argmax(abs(ovlp_mindiag)),ovlp_mindiag.shape)
                ...
                print("Smallest Overlap matrix eigenvalues",smallest_eigvals,
                         "Biggest Overlap matrix element", np.max(abs(ovlp_mindiag)), "at",max_indices)
        try:
            if include_overlap_correction: #This is the correction to the gradient that ensures that the Gaussians don't become linearly dependent
                beta=1e-20# "Small fraction of the energy"
                ovlp_punish=0.99
                o=overlapmatrix
                o_abs=np.abs(o)
                ...
                overlap_error=0
                prefac=beta/(1-ovlp_punish**2)
                for i in range(Ngauss_tot):
                    for j in range(i):
                        if i==j or (i<self.nfrozen and j<self.nfrozen):
                            continue
                        
                        if o_abs[i,j]>ovlp_punish:
                            print("Overlap between Gaussians %d and %d is %f"%(i,j,o_abs[i,j]))
                            overlap_error+=prefac*(o_abs[i,j]**2-ovlp_punish**2) #This is the penalty term
                            
                            #Next, we have to update the gradient for gaussians i and j by addin the derivative of the penalty term
                            #print(aderiv_funcs.shape)
                            ...
                            gradient[4*i-4*self.nfrozen]+=2*prefac*np.real(o[i,j]*aideriv)
                            gradient[4*i+1-4*self.nfrozen]+=2*prefac*np.real(o[i,j]*bideriv)
                            gradient[4*i+2-4*self.nfrozen]+=2*prefac*np.real(o[i,j]*pideriv)
                            gradient[4*i+3-4*self.nfrozen]+=2*prefac*np.real(o[i,j]*qideriv)
                            if j>=self.nfrozen:
                                ...
                                gradient[4*j-4*self.nfrozen]+=2*prefac*np.real(o[j,i]*ajderiv)
                                gradient[4*j+1-4*self.nfrozen]+=2*prefac*np.real(o[j,i]*bjderiv)
                                gradient[4*j+2-4*self.nfrozen]+=2*prefac*np.real(o[j,i]*pjderiv)
                                gradient[4*j+3-4*self.nfrozen]+=2*prefac*np.real(o[j,i]*qjderiv)
                sys.exit(0)
...
class Rothe_propagation:
    ...
    def propagate(self,t,maxiter,last_added=False):
        ...
        if optimize_untransformed:
            ...
            new_rothe_error,newgrad=optimization_function(solution,include_overlap_correction=False)
            ...
            new_lincoeff=rothe_evaluator.optimal_lincoeff
            new_rothe_error,gradient=optimization_function(solution,calculate_overlap=True,printing=True,include_overlap_correction=False)
        ...
        solution_reshaped=solution.reshape((-1,4))
        if (toobig or (removal and len(solution)//4>(n_extra-2)) and self.t>10) and (len(solution)//4)<50:
            ...
            avals_sample=get_Guess_distribution(avals,number_of_randoms);bvals_sample=get_Guess_distribution(bvals,number_of_randoms)
            pvals_sample=get_Guess_distribution(pvals,number_of_randoms);muvals_sample=get_Guess_distribution(muvals,number_of_randoms)
